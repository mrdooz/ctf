...
def get_message(html, tag='MESSAGE'):
    ss = '----- BEGIN %s -----' % tag
    ee = '----- END %s -----' % tag
...
def challenge_56():
    with requests.Session() as s:
        ...
        r = s.get('http://ringzer0team.com/challenges/56')
        msg = get_message(strip_tags(r.text), 'HASH')
        for i in range(10000):
            ...
            if msg == digest:
                print '** FOUND **'
                url = 'http://ringzer0team.com/challenges/56/%s' % n
                p = s.get(url)
